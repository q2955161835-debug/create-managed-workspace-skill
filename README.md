# Create Managed Workspace（创建托管工作区）

![Create Managed Workspace cover（创建托管工作区封面）](assets/cover.png)

Create Managed Workspace（创建托管工作区）是一个面向 Codex（代码代理）的 Skill（技能），用于快速创建可长期维护的标准工作区。它适合那些会反复发生、但每次目标不同的任务类型，例如 CAD（计算机辅助设计）建模、爬虫数据采集、研究写作、自动化脚本、文件转换和多产物项目。

它的目标不是生成某一种业务代码，而是先把工作区治理结构搭好：规则入口、项目地图、任务索引、进展记录、临时测试区、最终产物区、常用 Skill（技能）收纳位，以及每个小项目自己的 `try/` 调试目录。

## 为什么需要它

很多 Agent（代理）任务不是一次性问答，而是跨会话、跨文件、跨产物推进的项目。如果没有固定结构，常见问题包括：

- 产物散落在根目录，过几天不知道哪个是最新版。
- 临时测试脚本和正式交付物混在一起。
- 多个任务共用一个进展记录，后续接手困难。
- CAD（计算机辅助设计）源码、导出文件、截图和核验记录无法对应。
- 爬虫项目缺少变量确认表、来源记录和任务清单，重复抓取同一来源。
- 常用 Skill（技能）没有固定收纳位置，每次都要重新寻找。

这个 Skill（技能）把这些约束变成可执行脚本和可校验结构。
...
把本仓库放到 Codex（代码代理）可读取的 Skill（技能）目录中。例如：
...
如果你维护自己的外部 Skills（技能）库，可以把目录复制到该库，并在你的路由 Skill（技能）或说明书中登记：
...
如果希望脚本为新工作区初始化 Git（版本控制）并创建基线提交：
...
| 脚本 | 用途 |
| --- | --- |
| `scripts/new_workspace.py` | 创建标准托管工作区根目录。 |
| `scripts/New-Workspace.ps1` | Windows PowerShell（微软命令行脚本）包装器，调用 `new_workspace.py`。 |
...
建议每个 CAD（计算机辅助设计）任务都放在 `tasks/YYYYMMDD-短任务名/` 中：

- `input/`：图纸、照片、PDF（便携式文档格式）、参考尺寸。
- `work/`：参数化建模源码、草稿、临时检查脚本。
- `output/`：最终源码、STEP/STL/3MF/GLB/DXF/DWG、截图和核验记录。
- `try/`：可随时清理的几何试验。

如果 CAD（计算机辅助设计）Skill（技能）存放在外部路径，只在 `doc/项目地图.md` 中记录路径，不必复制进工作区。只有确实需要工作区私有版本时，才放入根目录 `skills/`。
...
### Research, Writing, Automation（研究、写作、自动化）

- `input/`：题目、素材、参考资料、用户输入。
- `work/`：草稿、脚本、Notebook（笔记本）、中间报告。
- `output/`：最终文档、表格、脚本包或交付文件。
- `try/`：一次性转换测试、命令探针、临时验证。

## 对既有工作区的整理原则

本 Skill（技能）第一版只创建新工作区，不自动迁移旧工作区。整理既有工作区时，应先阅读：
...
禁止把真实密钥、token（令牌）、cookie（会话凭据）、数据库密码或私有地址写入：
...
说明：如果 `SKILL.md` 包含中文，在部分 Windows Python（解释器）环境中需要设置 `PYTHONUTF8=1`，否则默认 GBK（中文编码）可能导致解码失败。
...
本项目当前未附带单独许可证文件。复用、分发或发布到团队环境前，请先按你的项目要求补充许可证。

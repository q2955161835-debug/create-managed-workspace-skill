...
它的目标不是生成某一种业务代码，而是先把工作区治理结构搭好：规则入口、项目地图、任务索引、进展记录、临时测试区、最终产物区、常用 Skill 收纳位，以及每个小项目自己的 `try/` 调试目录。
...
- `skills/`：工作区常用 Skill 收纳位，新建时保持空目录，只放 `.gitkeep`。
- `tasks/`：每个小项目的主目录。
...
如果 CAD Skill 存放在外部路径，只在 `doc/项目地图.md` 中记录路径，不必复制进工作区。只有确实需要工作区私有版本时，才放入根目录 `skills/`。

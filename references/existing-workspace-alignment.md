...
- First inspect current `AGENTS.md`、`doc/任务索引.md`、`doc/进展记录/`、Git status, and top-level directories.
- If a root `doc/项目地图.md` exists, treat it as deprecated and migrate its useful long-lived workspace information into `AGENTS.md` before removing it.
...
- Root has `AGENTS.md`、`.gitignore`、`.env.example`、`doc/任务索引.md`、`doc/进展记录/`、`try/`.
- Root does not keep `doc/项目地图.md`; the equivalent long-term workspace information belongs in `AGENTS.md`.
...
- Every small project has `doc/项目地图.md`、`doc/验收标准.md`、`doc/进展记录/` and `try/`.
...
- Treat CAD workspaces as `specialized` only when they need reusable CAD skills, wrappers, or skill entrypoints inside the workspace. Otherwise document external skill paths in `AGENTS.md` or the relevant task `doc/项目地图.md` without adding root `skills/`.
...
- Align each target by adding `doc/项目地图.md`、`doc/验收标准.md`、`doc/进展记录/` and `try/` when missing.
